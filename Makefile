...
# `cmd/pulumi-resource-ctfdcm/schema.json` is valid and up to date.
...
bin/$(PROVIDER)-v$(VERSION_GENERIC)-%.tar.gz:
	@mkdir -p dist
	@# $< is the last dependency (the binary path from above) e.g. bin/linux-amd64/pulumi-resource-ctfdcm
	@# $@ is the current target e.g. bin/pulumi-resource-ctfdcm-v1.2.3-linux-amd64.tar.gz
